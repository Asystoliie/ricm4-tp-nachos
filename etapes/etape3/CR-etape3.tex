...
\begin{tabular}{|lc|}
\hline 
status :& JUST\_CREATED, RUNNING, READY, BLOCKED \\ \hline 
stack :& L'emplacement (bas) de la pile \\ \hline
stackTop :& Sommet de la pile \\ \hline
machineState :&Le contenu de l'ensemble de registres \\ \hline
userRegisters : &Le contenu de chaque registre (Thread user) \\ \hline
space :& L'espace d'adressage (Thread user) \\ \hline
\end{tabular} \\
\vspace{0.2cm} \\
Un thread est tout d'abord créé avec la méthode Thread(). Elle initialise la
pile et affecte JUST\_CREATED à l'attribut status. Cet état temporaire indique
